...
export function IssueNode({ id, data }: NodeProps<PipelineFlowNode>) {
  ...
  return (
    <div
      ...
      data-testid="pipeline-node-issue"
      role="button"
      tabIndex={0}
      onClick={() => data.onClick?.({ id, type: "issue-node", data })}
      onKeyDown={(event) => {
        if (event.key === "Enter" || event.key === " ") {
          event.preventDefault();
          data.onClick?.({ id, type: "issue-node", data });
        }
      }}
    >
      <Handle type="target" position={Position.Top} isConnectable={false} />
      <div className="mb-2 flex items-center justify-between gap-3">
        <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Issue</span>
